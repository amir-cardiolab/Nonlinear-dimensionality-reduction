...
def performLEM(X, k_NN, r, gamma, transpose_flag):
# Perform the Laplacian Eigenmaps (Spectral Embedding) algorithm with scikit-learn
# Input:
# X - data matrix
...
# Output:
# X_lem - Spectral Embedding object
#       - X_lem.embedding_ contains the embedded vectors
    ...
    # knn_embedding = SpectralEmbedding(n_components = r, affinity = 'nearest_neighbors', n_neighbors = k_NN)
    # X_LE_knn = knn_embedding.fit(X)
    # affinity_knn = X_LE_knn.affinity_matrix_


    # convert from sparse to dense matrix
    # affinity_knn_dense = scipy.sparse.csr_matrix.todense(affinity_knn)
    # replace 0.5 to 1 (cases where i is knn of j, but j is not knn of i)
    # subtract 1 from the diagonal to make it zero
    # affinity_round = np.ceil(affinity_knn_dense) - np.diag(np.ones(affinity_knn_dense.shape[0]))
    
    
    affinity_knn = kneighbors_graph(X, k_NN-1, include_self = False)
    affinity_knn = np.ceil(scipy.sparse.csr_matrix.todense(0.5 * (affinity_knn + affinity_knn.T)))
    
    
    
    # compute the gaussian/heat kernel
    # rbf_embedding = SpectralEmbedding(n_components = r, affinity = 'rbf', gamma = gamma)
    # X_LE_rbf = rbf_embedding.fit(X)
    # affinity_rbf = X_LE_rbf.affinity_matrix_
    affinity_rbf = rbf_kernel(X, gamma = gamma)
    
    
    # multiply element-wise the two affinity matrices
    # all values will be zero where i is not a neighbor of j and vice-versa
    affinity_mat = np.array(np.multiply(affinity_knn,affinity_rbf))


    embedding = SpectralEmbedding(n_components = r, affinity = 'precomputed', random_state = 42)

    X_lem = embedding.fit(affinity_mat)
    end = time.time()

    print('Time elapsed for LEM:',end - start, ' s')
    print('Shape after embedding:', X_lem.embedding_.shape)

    return X_lem


def save_modes(X_lem, mesh, output_file, modesToSave, field_name):
...
# field_name - velocity field name in the original vtk files (just for removing unnecessary data)
    ...
    if modesToSave > X_lem.embedding_.shape[1]:
        modesToSave = X_lem.embedding_.shape[1]
        print('Max number of modes is', X_lem.embedding_.shape[1])
    
    print('Saving the first',modesToSave, 'LEM modes to ',out_filename, flush = True)

    for i in range(0,modesToSave,1):
        U_i = X_lem.embedding_[:,i]
...
def inverseTransform(X, X_lem, r_max, r_step, k_NN, reg, transpose_flag):
...
# k_NN - number of nearest neighbors used for LEM (same as for the performLEM function)
# reg - regularization parameter (same as for the performLEM function)
# Output:
...
# field_name - velocity field name in the original vtk files (just for removing unnecessary data)
    ...
    plt.title('LEM reconstruction error')
    plt.semilogy()
    plt.tight_layout()
    plt.savefig('LEM_rec_error.png',dpi = 200)
    if(plotToScreen):
        plt.show()
...
#Laplacian Eigenmaps w/ scikit learn

# k_NN - number of nearest neighbors used for LEM
...
X_lem = performLEM(X, k_NN, r_max, gamma, transpose_flag)


if transpose_flag:
    out_filename = 'LEM_modes.vtk'
    modesToSave = 8
    save_modes(X_lem, mesh, out_filename, modesToSave, field_name)

# Reconstruction
# r_step - mode number increment for calculating the reconstruction error
r_step = 1

X_reconstructed, err_rec = inverseTransform(X,X_lem, r_max, r_step, k_NN, reg, transpose_flag)
...
if save_rec_flag == True:
    ...
    reconstruction_filename = reconstruction_dir + '/reconstruction_LEM_'
