...
for epoch in range(num_epochs):
    ...
    for x in data_loader:
        ...
        if epoch == num_epochs-1:
            outputs.append((epoch, batch_iter, snapshot, recon, low_dim))
        batch_iter += 1
...
print('Done! - Time elapsed for Autoencoder:',end - start)
...
if save_rec_flag == True:
    ...
    reconstruction_filename = reconstruction_dir + '/reconstruction_T_AE_'

if(save_rec_flag):
    out_filename = 'Reconstruction/T_AE_reconstruction'
    ...
    for j in range(0,X.shape[1]):
        ...
        if j==0:
            for i in range(embed_out.shape[1]):
                meshNew.PointData.append(embed_out[:,i], 'mode_'+str(i))
        if case == 'turbulent_channel/':
