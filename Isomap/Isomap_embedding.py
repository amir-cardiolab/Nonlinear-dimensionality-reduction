...
def performIsomap(X, k_NN, r, transpose_flag):
...
# Output:
# X_isomap - Isomap object
#       - X_isomap.embedding_ contains the embedded vectors
    start = time.time()
    ...
    # Temporal reduction (transpose_flag = True) needs a much longer computational time than spatial reduction (transpose_flag = False)
        X = X.T
    X_isomap = embedding.fit(X)
    end = time.time()

    print('Time elapsed for Isomap:',end - start, ' s')
    print('Shape after embedding:', X_isomap.embedding_.shape)

    return X_isomap


def save_modes(X_isomap, mesh, output_file, modesToSave, field_name):
...
# field_name - velocity field name in the original vtk files (just for removing unnecessary data)
    ...
    if modesToSave > X_isomap.embedding_.shape[1]:
        modesToSave = X_isomap.embedding_.shape[1]
        print('Max number of modes is', X_isomap.embedding_.shape[1])
    
    print('Saving the first',modesToSave, 'Isomap modes to ',out_filename, flush = True)

    for i in range(0,modesToSave,1):
        U_i = X_isomap.embedding_[:,i]
...
def inverseTransform(X, X_isomap, r_max, r_step, k_NN, reg, transpose_flag):
...
# k_NN - number of nearest neighbors used for Isomap (same as for the performIsomap function)
# reg - regularization parameter (same as for the performIsomap function)
# Output:
...
# field_name - velocity field name in the original vtk files (just for removing unnecessary data)
    ...
    plt.semilogy()
    plt.tight_layout()
    plt.savefig('Isomap_rec_error.png',dpi = 200)
    if(plotToScreen):
        plt.show()
...
X_isomap = performIsomap(X, k_NN, r_max, transpose_flag)


if transpose_flag:
    out_filename = 'Isomap_modes.vtk'
    modesToSave = 8
    save_modes(X_isomap, mesh, out_filename, modesToSave, field_name)

# Reconstruction
# r_step - mode number increment for calculating the reconstruction error
r_step = 1

X_reconstructed, err_rec = inverseTransform(X,X_isomap, r_max, r_step, k_NN, reg, transpose_flag)
...
if save_rec_flag == True:
    ...
    reconstruction_filename = reconstruction_dir + '/reconstruction_Isomap_'
